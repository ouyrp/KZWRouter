...
@interface KZWImageCacheManager () {
    NSMutableDictionary *_imageCache; //使用这个字典保存使用过的图片，既加载到内存中的图片
}

@end

@implementation KZWImageCacheManager

+ (instancetype)sharedImageCacheManager {
    static KZWImageCacheManager *imageCacheManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        imageCacheManager = [[KZWImageCacheManager alloc] init];
    });
...
- (void)getImageWithUrl:(NSString *)urlString complete:(complete)complete {
    //1.从内存中加载图片
    UIImage *image = [[KZWImageCacheManager sharedImageCacheManager] imageFromDictionary:urlString];
    
    if (image) {
        complete(image);
    } else {
        //内存没有图片，就从本地读区图片
        image = [[KZWImageCacheManager sharedImageCacheManager] imageFromLocal:urlString];
        if (image) {
            complete(image);
        } else {
            //从本地没有读取到图片，从网络下载
